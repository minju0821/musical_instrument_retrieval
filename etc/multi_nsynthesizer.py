...
class NSynthesizer():
    ...
    def partially_render_sequence(self, sequence, instrument='guitar', source_type='acoustic', preset=None, playback_speed=1, duration_scale=1, transpose=0, eps=1e-9, start_position=0, duration_in_seconds=5.0, min_notes=3):
        preset = preset if(preset is not None) else self.preset
        ...
        # If there is nothing inside the seq, then return None
        if len(seq) == 0:
            return None
