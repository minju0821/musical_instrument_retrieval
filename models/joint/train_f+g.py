...
def parse_args():
    ...
    parser.add_argument("--save_dir_num", type=str, default='valid_fixed_03')
...
def train(models, opts, loss, train_loader, epoch, args, DEVICE):
    ...
    for batch_idx, (mix_audio, single_list, single_idxes, track_len) in tqdm_bar:
        ...
        training_time.append(time.time() - train_start)
        ...
        total_loss = f_loss_class + 5 * g_loss
        total_loss.backward()
        loss_time.append(time.time() - loss_start)
        ...
        if args.wandb:
            wandb.log({
                ...
                "loading time" : np.mean(loading_time),
                "training time" : np.mean(training_time),
                "loss time" : np.mean(loss_time)
            })
    ...
    torch.save(models['f'].state_dict(), f'/data4/aiproducer_inst/haessun_models/joint/{args.save_dir_num}/f_enc/epoch_{epoch}_totalLoss_{total_loss:.3f}.pt')
    torch.save(models['g'].state_dict(), f'/data4/aiproducer_inst/haessun_models/joint/{args.save_dir_num}/g_enc/epoch_{epoch}_totalLoss_{total_loss:.3f}.pt')
...
def evaluate(models, losses, valid_loader, lib_loader, epoch, args, DEVICE):
    ...
    with torch.no_grad():
        ...
        for idx, (mix_audio, single_list, single_idxes, track_len) in tqdm_bar:
            ...
            g_out = models['g'](mix_audio)[0]
            g_out = torch.reshape(g_out, (mix_audio.size()[0], 9, -1))
            
            #### estimated one hot ####
            for i in range(mix_audio.size()[0]):
                est_one_hot = np.zeros(53, dtype=int)
                for idx, est in enumerate(g_out[i]):
            ...
            #### answer one hot ####
            single_idxes = single_idxes.cpu().detach().numpy()
            l_before = 0
            for l in track_len:
                ans_one_hot = np.zeros(53, dtype=int)    
                ans_one_hot[single_idxes[l_before : l_before + l]] = 1
                l_before += l
...
if __name__=='__main__':
    ...
    # fewshot처럼 보이겠지만 oneshot입니다.
    lib_dataset = FewShotReprDataset(split='valid', num_few=1)
