...
class PITLossWrapper(nn.Module):
    ...
    def forward(self, est_targets, targets, track_num, return_est=False, reduce_kwargs=None, **kwargs):
        ...
        pw_losses[pw_losses==0] = -1 * 1e5
        pw_losses = 1 - pw_losses
        ...
        for b in range(targets.shape[0]):
            ...
            for j in range(targets.shape[1]):
                losses.append(pw_losses[b, j, batch_indices[b, j]])
            losses_sorted = sorted(losses)[:track_num[b]]
            min_loss[b] = torch.mean(torch.stack(losses_sorted))
        mean_loss = torch.mean(min_loss)
        mean_loss.requires_grad_(requires_grad=True)
        return mean_loss
    ...
    def find_best_perm_hungarian(pair_wise_losses: torch.Tensor):
        ...
        batch_indices = torch.tensor(np.array([linear_sum_assignment(pwl)[1] for pwl in pwl_copy])).to(
            pwl.device
        )
